...
|---|---|---|---|
| **M1 Vendor-in-git** | BMad copied into the shared repo by a deterministic vendor tool; team pulls | Proven by dps-council G2 (`vendor_bmad.py`: rules-not-patches, 8-check verify) | You own upstream patches (the plugin-hosting resolver fork); bundle bloat; re-vendor per release |
...
| **M3 Plugin marketplace** | Council (and optionally BMad) delivered as a Claude Code plugin from a git marketplace; update = git pull + version bump | Machine-wide single install; clean update UX; dps-proven | If BMad rides inside: inherits M1's fork. If council-only: must pair with M1 or M2 for the BMad base anyway |
...
**Seed's evaluation plan:** measure M1's real per-release cost (run the dps 6.9→6.10 vendor cycle) against an M2 prototype before deciding. As of 2026-07-25 this experiment is live: BMad v6.10.0 shipped July 3, so that vendor cycle can now be measured for real.
...
New subsystem #1 — no dps precedent. **Build order (Tim's ruling, 2026-07-25): local-first.** The default in-repo backends ship and stabilize before any external backend, making local development easy; migration local → online is designed to be straightforward.
...
## A3. Harvest map from dps-council
...
- Three-tier memory contract + per-user store design (`target-state-architecture.md` §3–6; `lib/dps_store.py` on the unmerged branch) → Taxonomy working data + project KB. The migration-safety rules — closed inventory, no-delete-before-clearance, copy-verify-flag cutover — port verbatim.
...
**LEAVE (dps-specific, future overlay):** the 11 personas and their lore; Perforce stream skills (`dps-stream-*`); changelog/newsletter skills; UE/Jenkins specialist content; canonical/ue garden.

## A4. Relationship to dps-council (context)

- tk-studio (working title "tk-council" in earlier documents) is **not** an import or restructuring of dps-council; it is a new, generalized personal system harvesting dps-council's proven concepts.
- The dps-council packaging debate (eval doc `eval-dps-council-packaging-2026-07-25.md` §4) is **superseded** by this direction; that eval's inventory and debts sections still stand for the dps-council repo itself.
- Long-term, dps-council re-bases as a thin game-company overlay on tk-studio.
- Inventory (2026-07-25) headline: plugin v0.7.1, 1,543 files — 17 dps-* skills (11 persona + 6 workflow), 71 bmad-* + 33 gds-* vendored skills, 11 agent wrappers, 5 domain profiles + detect/onboard scripts. It also **reconfirmed the eval's urgent flag**: `shared-memory/` contains a token leak — scrub before any harvest or publication. Per-user store code remains stranded on the unmerged branch (`lib/` holds stale bytecode only).
- Operational note: no live dps-council checkout exists on this machine — the inventory's source of record is `D:\ClaudeOS\dps-council.rar` (repo `main` @ d26a67f); the real working copy lives on another machine (Perforce workspace). Plan harvest work accordingly.
...
Added by Tim during brief review, 2026-07-25. New subsystem #3; precedent is ClaudeOS itself (mission runner + Dream cron), not dps-council.
...
- dps-council precedent — **VERIFIED (2026-07-25 inventory): the process EXISTS and was built**, as the "Research→Knowledge Lifecycle": `dps-investigate` generates a research spine at mission start (plus per-mini-goal seeds, anchored evidence-graded findings); working segments emit anchored deltas via handoffs → JSONL reconciliation queue → promotion gate owned by **Reina (custodian — not tech-writer; Sora is tech-writer, Yui is planner/decompose-gate)**. Wired into the ClaudeOS runner (`mission-spine-pass.ts`, `mission-mg-start-pass.ts`, `knowledge-schema.ts`, `mission-reconciliation-queue.ts`). Now on A3's PORT list; full detail in [dps-council-inventory-2026-07-25.md](dps-council-inventory-2026-07-25.md).
...
- Perforce note: the dps development process uses **task streams** mirroring git's feature-branch + commit-often approach — same discipline, different mechanics. Council conventions that assume git semantics (branch-per-goal, frequent commits, review-before-merge) must be expressible per-VCS, not hardcoded.
...
- Product naming (settled 2026-07-25): **tk-studio**, with `tk-studio-*` as the agent/skill prefix; "council" survives as the interaction mechanism (convene the council — see O5 persona shell). All references to "tk-council" in earlier-dated documents (the seed, the dps eval) are the working title of this same product.
...
Research inputs — both landed 2026-07-25 in this workspace: [dps-council-inventory-2026-07-25.md](dps-council-inventory-2026-07-25.md) (full plugin inventory; anti-drift process verdict: EXISTS, built) and [planning-backend-research-2026-07-25.md](planning-backend-research-2026-07-25.md) (tracker/KB landscape; v1 = markdown + frontmatter schema, one-way promote for migration).
