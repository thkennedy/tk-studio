# tk-council Plugin Inventory + Anti-Drift Tooling Findings — 2026-07-25

Input to the tk-council/tk-studio product brief. Read-only investigation.

- **Source inspected:** `D:\ClaudeOS\tk-council.rar` (full extract of the marketplace
  repo at `main` @ `d26a67f`, incl. git-ignored trees), extracted for this pass to
  `...\scratchpad\tk-council\tk-council`. No live checkout exists on this machine —
  the real repo home is the Perforce working dir on the other machine
  (`C:\Users\kenne\Perforce\tk-council`, git origin: the pre-rename upstream `claude-plugins`).
- **Companion doc:** `D:\ClaudeOS\_bmad-output\council\eval-tk-council-packaging-2026-07-25.md`
...
Repo = the `tk-local` marketplace hosting one plugin, **tk-council v0.7.1**
(`plugins/tk-council/.claude-plugin/plugin.json`). Extract totals: 9,270 files
...
|---|---|---|
| `plugins/tk-council/` | 1,543 | The plugin itself (see 1.2) |
...
### 1.2 The plugin (`plugins/tk-council/`)

| Piece | Count | Notes |
|---|---|---|
| `skills/tk-*` | 17 dirs (189 files incl. `__pycache__`) | The council IP: 11 persona-agent + 6 workflow skills |
...
| `agents/` | 11 files | ~1 KB delegation wrappers per persona ("invoke Skill `tk-council:<name>` as your FIRST action" → rebirth ritual runs). Not `skills:` preloads, deliberately |
| `bmad/` | 38 files | Vendored BMad framework modules: `bmb bmm cis gds tea reference scripts _config` |
| `lib/` | 3 files | ONLY stale `__pycache__` bytecode (`tk_store.cpython-312.pyc`, tests). The per-user store SOURCE lives only on unmerged branch `feat/mission-single-bmad-instal-stand-up-per-user-council-store-generali` — stranded |
...
| Skill | Persona | Role |
|---|---|---|
| `tk-agent-orchestrator` | **Alya** | Front door; data-driven routing from `domain-profiles/<council_domain>.json`; convene/synthesize/curate; domain onboarding |
| `tk-agent-task-planner` | **Yui** | Ticket → fully-scoped AC, story breakdown, tech specs (references: scope-ticket, break-down, draft-ac, tech-spec, prior-art) |
| `tk-agent-custodian` | **Reina** | Memory custodian; SOLE writer to canonical KB; promotion gate; drift observation (scripts: integrity-check, build-canonical-index, scan-ascii) |
| `tk-agent-tech-writer` | **Sora** | Docs: module READMEs, guide distillation, API docs, PR descriptions |
| `tk-agent-game-dev` | Raven | UE5 C++/BP gameplay, VR |
| `tk-agent-backend-dev` | Hina | Go modules for Kraken backend |
| `tk-agent-be-code-reviewer` | Rei | Backend review, rubric-driven |
| `tk-agent-ue-code-reviewer` | Kai | UE/Perforce CL review via sub-agent |
| `tk-agent-ops` | Mei | Edgegap/Nakama ops, audit-logged |
| `tk-agent-build-engineer` | Suzune | UE build + Jenkins pipeline |
| `tk-agent-tools-backend` | Kita | UE↔backend integration, telemetry, editor tooling |
...
| Skill | Purpose |
|---|---|
| `tk-investigate` | Evidence-graded research pass producing the mission spine/seed KB (see Q1) |
| `tk-stream-open` | Open isolated Perforce task stream + agent workspace for a ticket |
| `tk-stream-review` | Task-stream work → Swarm review (quality gate, merge-down check, copy-up shelf) |
| `tk-stream-land` | HUMAN-GATED land to `kraken_main` + post-land verification |
| `tk-changelog` | Player-facing changelog from Perforce CLs + GitHub PRs |
| `tk-update-newsletter` | Patch-notes newsletter reconciling changelog vs WIP team doc |

### 1.5 Domain-profile machinery (orchestrator skill)

- `skills/tk-agent-orchestrator/domain-profiles/{ue,be,tools,web,generic}.json` + `garden-templates/default-garden.json` + `README.md`
...
- `lib/` on main = bytecode only; per-user store (`~/.tk-council/`) implemented on an unmerged branch that is not an ancestor of main (eval D1).
- `skills/tk-agent-orchestrator/scripts/tests/__pycache__/test_check_plugin_version.cpython-312.pyc` — bytecode for a test whose SOURCE exists nowhere in the repo (eval D3). 14 `__pycache__` dirs shipped inside the plugin.
- `shared-memory/tk-agent-orchestrator/MEMORY.md` — token-suffix leak, scrub flagged urgent (eval D2).
...
- The pass itself (`tk-investigate`) is invoked automatically by the ClaudeOS mission runner; Alya routes manual runs.

**Evidence — council side (plugin repo):**

- `plugins/tk-council/skills/tk-investigate/SKILL.md` — the pass: two modes, budgets, anchor/evidence rules, "Never writes canonical", "promotion is Reina's gate".
- `plugins/tk-council/skills/tk-investigate/references/seed-template.md` — the artifact schema incl. the delta contract comment.
- Real artifacts produced by real missions: `.mission/adaptive-domain-onboarding-for-the-tk-c/g-mission-1783445603624-{2,3}/seed.md` and `.mission/single-bmad-install-council-memory-re-ho/mission-1784224747161-1/seed.md` (schema-conformant, anchored, evidence-graded).
...
accumulated in the tk-council extract (deltas queue in target repos as missions run).
...
1. **The mission knowledge KB (the Q1 machinery)** — spine + per-MG seeds under
   ...
   Explicitly designed as persistent state: tk-investigate step 1 says "Initialize the
...
3. **Persona sanctums (long-term memory)** — every persona: `MEMORY.md` + `INDEX.md` +
   ...
   cross-project (`shared-memory/tk-agent-orchestrator/`, 14 session logs); the 10
   specialists are project-local (`{project-root}/_bmad/memory/tk-agent-*/`). Reina's
...
Gap worth noting for tk-council: the designed **per-user store** (`~/.tk-council/`,
`lib/tk_store.py`) that would unify tiers 3's split memory models exists only on an
unmerged branch (eval D1) — on main, `plugins/tk-council/lib/` holds stale bytecode
