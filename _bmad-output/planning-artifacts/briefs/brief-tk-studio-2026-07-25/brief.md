...
_Name settled 2026-07-25 (working title was "tk-council"); skills and agents carry the `tk-studio-*` prefix. Seed: [tk-council concept seed, 2026-07-25](../../../council/tk-council-concept-seed-2026-07-25.md). Companion depth: [addendum.md](addendum.md)._
...
It is the deliberate generalization of **tk-council**, a game-company council system that proved the core concepts (domain detection, deterministic vendored distribution, tiered memory, orchestrated multi-agent routing) but entangled them with one studio's personas, Perforce streams, and Unreal Engine content. tk-studio extracts the engine from that vehicle. The long-term proof of success: tk-council itself re-bases as a thin game-company overlay on tk-studio.
...
- **Session and token discipline.** Long work splits at declared boundaries with compact handoff artifacts; runs persist in-progress state in resumable run workspaces so a fresh session continues without replaying history; token budgets are visible to the orchestrator when it routes work. Generalizes the ClaudeOS handoff protocol; the tk-council precedent is verified built — the "Research→Knowledge Lifecycle" (research spine before work, anchored deltas, custodian promotion gate) — and sits on the harvest PORT list (addendum A6).
...
- **Harvested, not speculated.** Nearly every piece has a working tk-council precedent — deterministic vendor tooling with a verify phase, the read-only detector, the tiered-memory contract, the Research→Knowledge Lifecycle, promotion gating. The one exception is the planning-data adapter, which is honestly new and therefore the highest-risk subsystem.
...
- **Tertiary — the future tk-council overlay:** the game-company layer (personas, Perforce skills, UE content) re-based on tk-studio, validating the generalization.
...
5. **Generalization proof:** tk-council's game-specific content can be expressed as an overlay without patching tk-studio core.
...
**Explicitly out:** game-company specialization (the 11 tk-council personas, Perforce stream skills, UE/Jenkins content, changelog/newsletter skills — all future tk-overlay material); any BMad fork beyond what O1 forces; multi-backend planning support beyond the first external backend; execution roles beyond developer (artist/designer/PR/custom roles arrive with the team); cross-project shared knowledge unless O4 pulls it in; a bespoke job-execution engine beyond what the chosen O8 substrate provides; the ClaudeOS MCP connector itself (ClaudeOS-side deliverable, built as a later step — tk-studio ships only the driver contract it consumes).
...
| O4 | Cross-project shared knowledge in v1? | in / out (per-project KB only) | tk-council "canonical" tier + promotion gate is the precedent; the Obsidian vault window (A2) partially serves the read side already | architecture |
