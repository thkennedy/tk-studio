...
- **Rule:** O2 ruling — **file-plane sync layer.** Stock BMad planning skills stay untouched and read/write their native artifacts; those artifacts, carrying the canonical frontmatter shape (AD-6), ARE the canonical local representation. The adapter is a standalone skill + library that the orchestrator sequences around planning flows (or the user invokes directly); it alone talks to external backends. No skill, studio or stock, ever calls a backend API/MCP directly.

### AD-5 — Sync semantics: one-way promote + status pull-back (O2)

- **Binds:** the planning adapter, all external backends
- **Prevents:** bidirectional-merge conflict hell the research showed nobody has solved
- **Rule:** local → backend is **promote** (create/update external items from canonical files); backend → local pulls **status-class fields only** (status, assignee, external key). Content authored locally wins locally; status observed externally wins locally. Any other divergence is surfaced as a conflict for a human — never silently merged. External identity lives under the entity's `external:` map, keyed by binding.
...
- **Rule:** O3 ruling — the first external planning backend is **Jira (Atlassian Cloud)** through the official Atlassian Remote MCP Server, shipped only after the local backends are stable, via the designed migration (AD-16). Headless flows MUST authenticate with API tokens, not OAuth (tk ISS-008: MCP OAuth cannot complete non-interactively). Confluence publishing of `kb/` and Linear are later adapters under the same contract.
...
- **Rule:** O4 ruling — **out.** Knowledge is per-project (`kb/`). Cross-project reads are served by the **project registry** in working data (the canonical path for "status of all projects") plus the Obsidian vault as the human window. The seam stays open: kb frontmatter reserves `scope:`, and the promotion-gate pattern (tk custodian membrane) is the named v2 mechanism. The evolve loop only observes and logs in v1.

### AD-9 — Stateless orchestrator core, optional persona shell (O5)

- **Binds:** the orchestrated entry point, all roles
- **Prevents:** identity/persona state entangled with routing logic (the tk lesson)
- **Rule:** O5 ruling — the orchestrator core is **stateless**: resolve role (from the per-user store) → resolve project working set (from project config) → route/convene/synthesize. The persona shell ("convene the council") is data — persona assets loaded only in attended sessions. Headless bypasses the shell entirely and must produce identical routing and artifacts. No sanctum/rebirth machinery in the studio core; that is tk-overlay material.
...
| Concern | Convention |
| --- | --- |
| Naming | Studio skills/agents: `tk-studio-*`, kebab-case verbs (seed roster: `activate`, `orchestrator`, `detect`, `onboard`, `plan-sync`, `migrate`, `report`, `measure-push`, `consolidate`, `base-update`, `job`). Entity ids `EP-/ST-/TA-NNN`; issues `ISS-NNN`; ids stable, never reused. Project key = root basename, `project_id` overrides on collision. |
...
| Vault window | `<vault>/projects/<name>/` is a real vault folder holding per-category Windows directory junctions (`kb` → `{project-root}/kb/`, `backlog` → planning folder) — created at onboarding, verified at activation. The vault is a view; the registry is the database. |
...
| Atlassian Remote MCP Server (first external backend) | hosted, GA 2026-02-04 |
...
| Taxonomy (stores, kb, vault window) | per-user store, `kb/`, junctions | AD-3, AD-8, AD-15, AD-18 |
...
| Operations (jobs, loops, dual-mode) | `tk-studio-job`, job definitions, `contracts/` | AD-10, AD-11, AD-14 |
