...
- **Rule:** O4 ruling — **out.** Knowledge is per-project (`kb/`). Cross-project reads are served by the **project registry** in working data (the canonical path for "status of all projects") plus the Obsidian vault as the human window. The seam stays open: kb frontmatter reserves `scope:`, and the promotion-gate pattern (legacy-council custodian membrane) is the named v2 mechanism. The evolve loop only observes and logs in v1 *(landed 2026-08-08, Epic 10: the drafting half now derives `proposals/ledger.md` PROP rows from accumulated observations at contract 0.1.9 — documents only, adoption human-initiated per D1; the cross-project boundary this rule carries is unchanged)*.
...
### AD-16 — Migration is a designed operation with legacy-council safety rules `[ADOPTED]`
