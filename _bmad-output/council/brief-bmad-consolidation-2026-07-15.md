...
Consolidate to a SINGLE BMAD install delivered via the dps-council plugin, shared across all projects, and re-home council memory onto a clean three-tier model. The team is migrating fully to Claude Code (dropping Cursor), so the non-Claude skill layer is now dead weight. Tim has run the dps-council plugin across all projects for a while and is confident in its stability.

## Locked decisions (do NOT reopen — Tim already decided these with Alya)

1. **BMAD core is BUNDLED INTO the dps-council plugin** — not a separate plugin, not machine-wide `~/.claude/skills`. One plugin delivers council specialists + BMAD core; they share a release cycle.
2. **Three-tier memory model, governed by "promotion is the membrane"** — everything a council session produces is per-user cognition until Reina promotes it; promotion is the ONLY path into anything team-shared (VCS):
   ...
   - **Tier 3b — Promoted DOMAIN-general knowledge** (Reina-gated): stays in `dps-council/canonical/<domain>` (existing model).
...
4. **Multi-user fix (critical):** today `dps_shared_root` points INTO the shared Perforce dps-council repo (`C:/Users/kenne/Perforce/dps-council/shared-memory`) and project pools live in `{project}/_bmad/memory/dps/`. For a team this collides. Tiers 1 & 2 must move to a PER-USER store OFF the shared repo (e.g. `~/.dps-council/`). The shared dps-council repo then holds ONLY Tier 3b (canonical).
...
- **THREE council generations coexist in project skill dirs; only the plugin one is live:**
  - gen1 (oldest): `dpsue-*` (kraken_main, 12 skills), `dpsbe-*` (Kraken-Backend, 16 skills)
  - gen2 (middle, pre-reviewer-split): unprefixed `dps-*` (all 4 repos, ~10 each)
  - gen3 (LIVE, keep): the plugin's `dps-agent-*` (17 skills), at `C:/Users/kenne/.claude/plugins/cache/dps-local/dps-council/0.5.0/skills/`
- **MEMORY HAZARD:** `Kraken-Backend/_bmad/memory/dpsbe` is the ONLY record of that project's council history (no new-gen dps pool there). kraken_main has `dpsue` + legacy pools alongside new-gen. These hold real state.

## Migration buckets (Alya first-pass — refine into mini-goals)

- **DELETE:** all `.agents/skills` (identical mirror); `.cursor/` dirs; the two dead council generations (`dpsue-*`, `dpsbe-*`, unprefixed `dps-*`).
...
- **MOVE out of project VCS:** `{project}/_bmad/memory/dps*` pools + per-project sanctums -> per-user council store.
- **MIGRATE-THEN-DELETE (gated):** Reina runs a promotion pass on Kraken-Backend's `dpsbe` pool and kraken_main's `dpsue` pool, promoting anything worth keeping into the respective project's VCS, BEFORE deletion.
- **RE-HOME:** `dps_shared_root` -> per-user path; establish the Tier-3a convention (where project-promoted knowledge lands in each repo, e.g. `docs/council/`).
...
**A. Agent-driven plugin update.** A skill/workflow (plus any needed hooks) to update the dps-council plugin — including its now-bundled BMAD version — via agent interaction. The agent performs/facilitates the update rather than the dev doing it by hand.
...
3. How "latest available" is discovered — the plugin source is `dps-local` (a local plugin marketplace/source at the dps-council repo).

## Constraints / rules

- **Target repo** for the mission is the dps-council repo (plugin + canonical live there). Several mini-goals also delete/move things in the 4 project repos.
