...
- **target_repo:** `C:\Github\ClaudeOS` (Tim, OQ-1). Reusable council-side artifacts — a canonical
  schema doc and a dedicated `tk-investigate` skill — are a **separate later track** in the tk-council
...
1. **OQ-1 repo:** ClaudeOS. Council-side reusable bits deferred to a later tk-council track.
2. **OQ-2 investigate:** the research pass defaults to **`tk-investigate`**, with a **pluggable seam**
   ...
   this session):** the `tk-investigate` skill now EXISTS at
   `C:/Users/kenne/Perforce/tk-council/plugins/tk-council/skills/tk-investigate/` (SKILL.md +
   ...
   in the tk-council git repo (uncommitted, Tim to commit). **Knock-on for mg1:** the seed/delta SCHEMA
...
3. **MG-start: decompose → notify-gate → seed pass** — cheap decompose; gate `#stories > MAX_STORIES_PER_MG`
   ...
   `tk-investigate` (pluggable). Ordering proven: no research before the gate. *(deps: mg1, mg2)*
...
4. `tk-investigate` binding — resolves to the council investigate capability until the dedicated skill
