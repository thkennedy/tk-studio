...
- **Supersedes** the legacy-council packaging debate scoped in
  ...
  sections still stand for the legacy council repo itself.
- **Relationship to the legacy council:** tk-council is NOT an import or
  restructuring of the legacy council. It is a new, generalized, personal system
  that harvests the legacy council's proven concepts. Long-term, the legacy council
  can be re-based as a thin game-company overlay ON tk-council (personas, Perforce
  stream skills, UE canonical content are exactly the layer that does not belong in
  the generic version).
...
|---|---|---|---|
| M1 Vendor-in-git | BMad copied into the shared repo by a deterministic vendor tool; team pulls | Proven by the legacy council's G2 (vendor_bmad.py: rules-not-patches, 8-check verify) | You own upstream patches (the plugin-hosting resolver fork); bundle bloat; re-vendor per release |
...
| M3 Plugin marketplace | Council (and optionally BMad) delivered as a Claude Code plugin from a git marketplace; update = git pull + version bump | Machine-wide single install; clean update UX; proven in the legacy council | If BMad rides inside: inherits M1's fork. If council-only: must pair with M1 or M2 for the BMad base anyway |
...
| Category | What it is | Scope | VCS? | Location | Legacy-council precedent |
|---|---|---|---|---|---|
...
| Council data | The skills, agents, workflows, orchestrator, profiles -- everything layered on core BMad | Cross-project, team-shared | YES (tk-council repo) | The tk-council git repo, delivered by the P1 mechanism | The legacy plugin's tk-* content |
...
Generalization of the legacy council's domain-profile registry from "fixed roster keyed by
...
- **Detect:** project type + technologies for brownfield (the legacy council's detect-domain.py is
...
- O4. Cross-project shared knowledge (the legacy council's "canonical"/T3b, Reina-style promotion
...
## 4. Harvest map from the legacy council
...
LEAVE (game-company-specific): the 11 personas and their lore; Perforce stream
skills (tk-stream-*); changelog/newsletter skills; UE/Jenkins specialist content;
canonical/ue garden. These become the future game-company overlay on tk-council.
...
2. `bmad-forge-idea` or party-mode debate on O1 (the lockstep mechanism) -- the one
   ...
   (the legacy council's 6.9->6.10 vendor run) vs an M2 prototype.
