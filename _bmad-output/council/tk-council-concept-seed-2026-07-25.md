...
- **Supersedes** the dps-council packaging debate scoped in
  `eval-dps-council-packaging-2026-07-25.md` section 4 -- Tim's 2026-07-25 direction
  answers its core question (see "Settled" below). That eval's inventory and debts
  sections still stand for the dps-council repo itself.
- **Relationship to dps-council:** tk-council is NOT an import or restructuring of
  dps-council. It is a new, generalized, personal system that harvests dps-council's
  proven concepts. Long-term, dps-council can be re-based as a thin game-company
...
| Category | What it is | Scope | VCS? | Location | dps-council precedent |
|---|---|---|---|---|---|
...
| Council data | The skills, agents, workflows, orchestrator, profiles -- everything layered on core BMad | Cross-project, team-shared | YES (tk-council repo) | The tk-council git repo, delivered by the P1 mechanism | The plugin's dps-* content |
...
Generalization of dps-council's domain-profile registry from "fixed roster keyed by
...
- **Detect:** project type + technologies for brownfield (dps detect-domain.py is
...
- O4. Cross-project shared knowledge (dps "canonical"/T3b, Reina-style promotion
...
## 4. Harvest map from dps-council
...
- Three-tier memory contract + per-user store design (target-state-architecture.md
  sections 3-6; lib/dps_store.py on the unmerged branch) -> P2 working data +
...
LEAVE (dps-specific): the 11 personas and their lore; Perforce stream skills
(dps-stream-*); changelog/newsletter skills; UE/Jenkins specialist content;
canonical/ue garden. These become the future dps overlay on tk-council.
...
2. `bmad-forge-idea` or party-mode debate on O1 (the lockstep mechanism) -- the one
   ...
   (the dps 6.9->6.10 vendor run) vs an M2 prototype.
