...
- **DW-3** — Contract wording updates to fold into the next deliberate bump (with DW-1): §8's assertion enumeration should name the suite's `unrunnable-core` assertion (added 2026-08-06 — every SKILL.md documents the canonical unrunnable-core discipline paragraph), and §6's "three-way" activation phrasing should read four-plane (base, plugin incl. harness loadability, store, vault). Both are PATCH-level wording; behavior already shipped and asserted by the suite.
