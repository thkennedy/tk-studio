...
|----|--------|--------|----------|----------|------------------|-------------------|-----|--------|---------|
| PROP-001 | Medium | Adopted | Upstream bmad-method 6.10.0 installer, on --yes reinstall over an existing install, re-serializes list-valued module config options as JSON strings (primary_platform [a,b] becomes a quoted string) and rewrites installed files with LF endings — no-op git churn after every verify run. | 1× observation from tim-Tim-PC (2026-07-27) ↔ ISS-001 | Teach tk-studio-base-update's verify step to auto-normalize the known churn classes (list re-serialization, LF rewrites) before diffing, and file the re-serialization defect upstream with bmad-method. | tk-studio-base-update | obs:upstream-bmad-method-6.10.0-installer-yes-reinstall-98dd56 | 2026-07-27 | 2026-07-27 |
