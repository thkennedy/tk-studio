...
| ISS-004 | Medium | Resolved | Version drift detected between installed planes and the pins | Expected: installed planes match the pins on every machine (AD-13). Actual: drift reported | 4× drift-detection from tim-Tim-PC (2026-08-09) | run the guided fix on the affected machine (tk install / /plugin marketplace update). Executed 2026-08-09: the 3 drift events are the ST-045 release window (installed 0.1.0 vs repo 0.2.0 — expected mid-release); closed same day by claude plugin install after the marketplace-update path left installPath unmaterialized (PROP-017); the final drift-detection in the same corpus is clean. | drift | 2026-08-09 | 2026-08-09 |
